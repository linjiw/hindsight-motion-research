"""Write the evidence report only after the registered native panel has completed."""
import argparse
import json
...
def report(root):
    root=Path(root)
    summary=json.loads((root/'aggregate.json').read_text())
    verification=json.loads((root/'verification.json').read_text())
    if summary['completed']!=summary['registered'] or not verification['passed']:
        raise ValueError('Complete verified panel required before writing results')
    rows=json.loads((root/'all_attempts_and_outcomes.json').read_text())
    panels=json.loads((root/'paired_relation_audit.json').read_text())
    critical=[r for r in rows if r['condition']=='critical']
    wide=[r for r in critical if r['variant']=='wide'];tight=[r for r in critical if r['variant']=='tuck']
    table=[]
    for name,label in [('critical','关键位置：0.76 m 通道'),('relaxed','放宽通道至 1.36 m'),
                       ('removed','移除障碍'),('displaced','障碍横移 2 m')]:
        c=summary['conditions'][name]
        table.append(f"| {label} | {c['tuck']['passes']}/{c['tuck']['completed']} | {c['wide']['passes']}/{c['wide']['completed']} |")
    text=f'''# 第一组真实物理仿真实验：动作为什么需要避障
...
三个初始位置为横向偏移 0、−1.5、+1.5 cm。**{sum(p['verified'] for p in panels)}/3 组完整干预面板通过关键性审计**：同组八次运行的初始 root、关节、关节速度和 930 维 proprioceptive history 完全一致；控制场景中两个动作都通过，关键场景中收臂通过、展臂与障碍发生手部接触。
...
- 收臂穿过关键通道：最大障碍接触力为 0 N；终点误差范围 {min(r['final_goal_distance_m'] for r in tight)*100:.2f}–{max(r['final_goal_distance_m'] for r in tight)*100:.2f} cm。
- 展臂在关键通道受阻：双手发生障碍接触，最大法向力范围 {min(r['max_obstacle_normal_force_n'] for r in wide):.1f}–{max(r['max_obstacle_normal_force_n'] for r in wide):.1f} N；终点误差 {min(r['final_goal_distance_m'] for r in wide)*100:.1f}–{max(r['final_goal_distance_m'] for r in wide)*100:.1f} cm。
...
已经导出 {verification['control_rows']:,} 个控制时刻，包含三种分离的数据视图：
...
{verification['scene_supported_motor_rows']:,} 行具有本次穿行任务的正向 imitation support；受阻 rollout 作为失败/关系标签保留，不进入正向动作支持集。RVQ motion code 与 SONIC motor token 是不同空间，需要单独学习和验证接口。全部数据当前属于 development；没有训练或报告 BFM/text2nav 的收益。
...
'''
    path=PROJECT/'docs/CRITICAL_RESULTS_zh.md';path.write_text(text)
    print(path)


if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('output',type=Path);a=parser.parse_args();report(a.output)
