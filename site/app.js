...
const decisions={
 execution:{title:'Three controls retained; the remaining case is resource deferred.',evidence:'Admission 02 closes normally after 300 seconds with zero launches. None of 15 samples qualifies; GPU free memory peaks at 11,218 MiB versus 12,000 required. Three successful controls are reused exactly; the old failure and prior SIGTERM receipt remain separate.',question:'Does accepting the pending Linear29 request within its legal window recover the farther-goal task?',test:'Check resource availability, then register only the same never-launched case in a fresh admission: at most one attempt / 500 ticks, within the original four-attempt total. Reuse the completed farther continuous control and keep all scientific settings fixed.',pivot:'The sibling’s calibrated continuous pending run now succeeds, but does not fill this codec cell. A local success would support a scoped temporal repair and unlock a compatible episode inventory; failure would redirect work to continuation or stopping support.'},
