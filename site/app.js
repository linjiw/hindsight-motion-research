...
const decisions={
 execution:{title:'Three old paths retained; delayed Linear29 execution is still unrun.',evidence:'The new pending wrapper preserves three complete native trajectories exactly. The decisive farther-Linear29 case never launched: the queue received SIGTERM during its memory wait. The old failure and all resource samples remain recorded.',question:'Does accepting the pending request within its legal window actually recover the farther-goal task?',test:'Use a separate admission only for the one never-launched case: at most one attempt / 500 ticks, within the original four-attempt total. Reuse the qualified farther continuous control; keep codec, predictor, motor, clearance and 3D task thresholds fixed.',pivot:'The shadow accepts at 134, with a proved last acceptance at 162, but cannot establish physical success. A successful delayed run would support this temporal interface in the tested task; failure would require separate loop-support and component diagnostics.'},
