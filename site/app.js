...
const decisions={
 execution:{title:'Both simple routes complete the prescribed task.',evidence:'New complete-task pilot: continuous 4/4 and Linear29 with reset anchors 4/4. Four paired initial states, histories and dynamics match exactly; complete future references remain supplied.',question:'Does this support survive switching from an actual incoming state?',test:'Eight paired pre-entry, entry, exit and pre-hold states are frozen. The runner and history/RNG audits are implemented, but the 300-second resource gate expired with zero native launches.',pivot:'If only a codec loses supported continuations, study its information loss. If both fail, repair switching support; if both pass, separately test new chunk choices and perturbations before a causal composer.'},
