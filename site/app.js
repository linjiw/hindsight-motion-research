...
const decisions={
 execution:{title:'The next comparison is prepared; all four cases remain unrun.',evidence:'The old six-case screen is complete at 1/3 each. The new distance-exit adapter replays 792 historical states exactly, but its 300-second resource gate expired with zero launches. The sibling separately reports 9/10 public versus 5/10 short-only, including one regression.',question:'Does raw Linear29 preserve the frozen distance controller’s actual choice and full stopping on its own visited states?',test:'Use a new admission for the original/farther beam × continuous/Linear29 registration, preserving its deferral and total four-attempt ceiling. Measure actual gate, choice, state parity and task completion under the declared velocity and forecast conventions.',pivot:'Codec-only failure motivates a targeted velocity or boundary control. The sibling’s stopping-prediction regression is a separate controller problem. No resource deferral, offline replay or shadow reference counts as a physical success.'},
