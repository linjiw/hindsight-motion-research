...
const decisions={
 execution:{title:'Both references pass the same-phase handoff panel.',evidence:'Complete-reference pilot: 4/4 per method. Follow-up: continuous 8/8 and Linear29 8/8, with eight exactly matched incoming states and histories. Prefix, phase, root and future remain supplied.',question:'Can the controller execute a new continuation selected from measured state and scene geometry?',test:'Build a small typed motion library, reuse the qualified sibling state/executor contract, then register actual selected transitions with continuous/Linear29 controls. Perturbations and causal from-reset episodes remain separate gates.',pivot:'A continuous transition failure points first to support or planning. A codec-only failure motivates representation diagnostics. Passing selected transitions supports moving to a causal composer; it does not establish generalization.'},
