...
const decisions={
 execution:{title:'The six-case screen is complete; both methods share its failures.',evidence:'Both pass the shift, contact the lower beam and time out at the farther goal: 1/3 each. Four new runs finish six total / 1,867 steps. The sibling now has a separate distance-exit controller; our 1,528-state codec preflight is offline only.',question:'Does Linear29 retain actual distance-dependent choice and full stopping on that newly supported controller?',test:'Register original/farther beam × continuous/Linear29 with fixed composer and shared scene bytes. Declare the one-tick forecast shift and blend-velocity convention; measure each method’s actual gate, choice, task and costs.',pivot:'If continuous also fails, inspect task/controller support. If only the codec fails, isolate velocity, temporal boundary and clipping effects. Same outcome categories in three development contexts do not establish equivalence or generalization.'},
