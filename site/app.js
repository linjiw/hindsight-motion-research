...
async function loadEvidence(){
 try{
  ...
  evidence=Object.fromEntries(entries);renderLab();renderComparison();renderScene();renderDecision(selectedDecision);
...
const decisions={
 execution:{title:'First ask whether the reference can be executed.',evidence:'Latest body9 qualifies 0/18; leg12 qualifies 15/18. Tested arm/beam clearance scores stay unchanged by leg replacement.',question:'Does the representation remove information needed for physical execution?',test:'Freeze the tracker and compare continuous, Linear29 and a candidate codec, including incoming-state transitions.',pivot:'If continuous also fails, investigate task/teacher support. If only the decoded reference fails, study representation loss.'},
 choice:{title:'A useful adjustment may still need no learned selector.',evidence:'Three of five canonical edited pairs pass every intervention panel. No navigation student has been trained here.',question:'Does scene context change the best complete behavior under a declared task cost?',test:'Compare the scene-conditioned planner with always-tuck/duck, ordinary walking and a simple geometry rule.',pivot:'If one constant behavior solves everything at comparable cost, broaden the task choices or keep the simpler controller.'},
