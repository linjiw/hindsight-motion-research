...
const interfaces={
 ...
 feedback:{title:'An agent needs measured completion and failure.',status:'Known-map complete-task selector measured; general agent feedback API proposed.',body:'Report progress, active, blocked, replan, unsupported or complete. Completion means the declared physical terminal condition. A clip ending does not prove stopping; a stop fallback also requires support from the current state.',schema:'Proposed execution feedback\nrequest + chunk ID + timestamp\nmeasured progress + task components\nobservation validity + failure reason\ncomplete only after the terminal gate'},
...
const decisions={
 execution:{title:'The shifted beam passes; four planned cases remain unrun.',evidence:'Full reference: 4/4 each; same-phase handoffs: 8/8; familiar selector: 2/2. New 20 cm shift: 1/1 each, 719 control steps. Four lower/farther cases are resource-deferred. On the new histories, a 60 cm farther goal changes zero references under either method in an offline check.',question:'Does Linear29 preserve the two pending controller failures, and can supported exit/stop composition respond to remaining distance?',test:'First finish the four pending comparisons under a separate admission. Prepare actual exit/stop segments and support seams, then require coherent goal response on common histories before new composed-reference execution.',pivot:'Keep codec failure distinct from missing motion coverage or goal responsiveness. A failed continuous control stays in the denominator. The current offline diagnostic is not farther-goal physics, and an intact language sidecar does not implement missing control behavior.'},
