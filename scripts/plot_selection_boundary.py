...
data = json.loads((root / 'results/selection_boundary.json').read_text())
...
fig.suptitle('Does the representation change the controller’s support boundary?', x=.025, ha='left', fontsize=15, fontweight='bold')
fig.text(.025, .89, f"PARTIAL SCREEN: {data['completed_episodes']} completed / {data['scheduled_cases']} scheduled; {data['unrun']} unrun after resource deferral", fontsize=11, color='#6c452b')
fig.text(.025, .04, f"{data['native_attempts']} new attempts · {data['control_steps']:,} control steps · zero training\n"
         'Three development contexts selected after a prior screen; one ancestry and seed. Unrun cases are not failures.\n'
...
fig.savefig(root / 'artifacts/selection_boundary.png', dpi=180)
plt.close(fig)
