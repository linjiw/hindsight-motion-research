...
**距离响应的 complete-task 主比较已完成：continuous 2/2，Linear29 1/2。** [admission 02 结果](DISTANCE_CODEC_ADMISSION02_RESULTS_zh.md)在原 frozen controller 上新增四次 / 1,660 control steps / 6,640 physics samples，零训练/重试。原目标两者成功；远目标 Linear29 正确请求 loop，却因 tick 126 时全身 clearance 差规定 2 cm 余量 1.069 mm 而保留 short，最终在 0.674383 m 处超时。四次无禁用接触或跌倒。continuous controls、配对 reset/history/dynamics/RNG、全部 scorer 与 issued-reference replay 均精确；原 0-launch receipt 不改写，原四次预算用完。
...
**该唯一 case 的 [admission 02 已登记并正常资源延期](PENDING_EXIT_ADMISSION02_RESULTS_zh.md)。** 428 项绑定、三条控制及 task bytes 全部保持；300 s 内 15 次采样零合格，GPU free 最高 11,218 MiB < 12,000，零新 native/steps/physics。旧 SIGTERM 与本次正常 timeout 分开保留，累计仍 3 次 / 1,160 steps，关键格未运行，92 tests 通过。下一步先只读确认资源可用，再另建 admission 03 补同一格；最多一次 / 500 ticks / 2,000 physics samples，总四次 ceiling 不变，不重跑 controls。保持 2 cm gate、0.50 m 3D goal、motor/codec/原 predictor 与 162 窗口。更远 forecast 可修订，134 shadow 与 +0.70 s q/qdot 变化仍无物理后续。

**[相邻 committed pending 结果](PENDING_CONTROLLER_SYNC_20260920_zh.md)已同步到 `2a3cfb47`。** Calibrated continuous 的 backward-start 在 138 接受后成功；early supplied-loop 也成功。这支持测共享时间接口，但不能替代原 predictor 的 Linear29 任务结果。若本格成功，再整理 controller/codec 兼容的完整 episode inventory、causal 输入/target mask、按 episode/reset/goal/祖先分组的小学习协议；失败则先定位 continuation/stop 支持。小 LLM 与更大 tokenizer 仍为次分支，不启动新训练。目标维度的早期误标见[更正](DISTANCE_METRIC_CORRECTION_20260919.md)。
