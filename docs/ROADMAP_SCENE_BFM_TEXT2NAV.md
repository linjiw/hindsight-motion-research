...
Both simple routes also complete the newly tested 20 cm earlier beam: 7.18 / 7.20 s, zero forbidden contact. This [six-case screen](SELECTION_BOUNDARY_RESULTS_zh.md) completed only two cases; the 300-second resource gate deferred the lower-beam control and left four cases unrun. The original tentative ±5 cm/shared-clear proposal was replaced before execution after reading the sibling's broader screen. Its known lower-ceiling and farther-goal failures motivated the selected diagnostic, not a held-out evaluation.

First finish the four pending continuous/Linear29 comparisons under a separate admission, preserving this closed packet and the two completed cases. On all 719 newly recorded states, an exploratory goal-only replay finds no candidate, index or reference response to extending the goal by 60 cm under either representation. This is not farther-goal physics. Prepare [distance-responsive exit and stopping](DISTANCE_RESPONSE_GATE_zh.md): qualify actual segment/support seams, require coherent reference response on common histories, then compare supplied composed support and public composition before learning. Lower ceilings need their own motion coverage and blocked-state qualification. Keep the continuous planning bank and all side-channel costs explicit. Incoming-state diversity, perception, held-out robustness and cross-family composition remain open.
