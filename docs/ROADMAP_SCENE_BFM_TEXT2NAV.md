...
The separately registered eight-attempt reference pilot is complete (2,079 control steps / 8,316 physics frames). Its budget is exhausted. The [incoming-state study](CONTINUATION_PROTOCOL.md) has now completed its linked admission: [continuous 8/8 and Linear29 8/8](CONTINUATION_ADMISSION02_RESULTS_zh.md), 16 native attempts, 4,136 control steps and 16,544 physics frames. All eight pairs match exactly; no forbidden contact or training. The original zero-launch resource deferral remains immutable. Same-phase, unperturbed reference handoffs are only the first subset of row 2; new chunk choices and perturbation recovery remain separate. The next work is a small primitive bank and measured-state selector, reusing the sibling interface as detailed in the [new sync](COMPOSER_SYNC_20260918.md). The historical ceiling still has 168/480 main attempts remaining. Reusing any of it requires an applicable registration and serial resource gate; a new training campaign requires a new budget. Do not turn a remaining ceiling into a target sample count.
...
## Current next step after the continuation panel

Both simple routes complete the selected known references and all eight same-phase handoffs. No complete-task success bottleneck has been isolated for Linear29 here. Proceed to selecting and executing new continuations from measured state and geometry before training a larger tokenizer. Build the typed primitive bank, qualify actual selected transitions with matched histories, then register perturbations and causal from-reset evaluation separately. The first pilot used the existing goal050 profile; do not silently relabel it goal025. Frozen Linear29 ranges clipped four wrist channels, so later failures need calibration-versus-temporal-loss controls. See the [complete-task report](COMPLETE_TASK_RESULTS_zh.md).
