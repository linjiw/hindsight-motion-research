...
The separately registered eight-attempt reference pilot is complete (2,079 control steps / 8,316 physics frames). Its budget is exhausted. The next [incoming-state study](CONTINUATION_PROTOCOL.md) is implemented and registered for sixteen attempts: two beam tasks × four frozen states × two methods. Its 300-second resource gate expired before launch; [all sixteen remain unrun](CONTINUATION_RESULTS_zh.md), with zero new native attempts. Preserve that receipt and use a linked admission packet when resources are available. Same-phase, unperturbed reference handoffs are only the first subset of row 2; new chunk choices and perturbation recovery remain separate. The historical ceiling still has 168/480 main attempts remaining. Reusing any of it requires an applicable registration and serial resource gate; a new training campaign requires a new budget. Do not turn a remaining ceiling into a target sample count.
