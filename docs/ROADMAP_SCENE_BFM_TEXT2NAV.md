...
| P1 — complete task baseline | Continuous × Linear29 across full-reference reset, matched-state suffix, and causal-composer reset conditions; native parity separately | Full reference: 4/4 per method; same-phase continuation: 8/8; familiar known-map selector: 2/2. Completed scene screen: 1/3 each—shift pass, lower-beam contact, farther-goal deadline. Distance-exit comparison: continuous 2/2, Linear29 1/2, a missed clearance decision. Matched codec comparisons under reset perturbations and cross-family composition remain untested |
...
The separately registered eight-attempt reference pilot is complete (2,079 control steps / 8,316 physics frames). Its budget is exhausted. The [incoming-state study](CONTINUATION_PROTOCOL.md) has now completed its linked admission: [continuous 8/8 and Linear29 8/8](CONTINUATION_ADMISSION02_RESULTS_zh.md), 16 native attempts, 4,136 control steps and 16,544 physics frames. All eight pairs match exactly; no forbidden contact or training. The original zero-launch resource deferral remains immutable. Same-phase, unperturbed reference handoffs are only the first subset of row 2; new chunk choices and perturbation recovery remain separate. The sibling subsequently supplied that bank and selector. Our [September 19 codec comparison](SELECTION_CODEC_RESULTS_zh.md) completes both familiar reset tasks per method: four new attempts, 1,451 control steps / 5,804 physics frames. Its continuous planning bank is retained in both arms; this tests motor-reference representation, not compressing the planner. The subsequent [scene-variation screen](SELECTION_BOUNDARY_ADMISSION02_RESULTS_zh.md) now completes all six cells across two admissions; four new attempts add 1,148 steps, with the two earlier runs reused. The historical ceiling still has 168/480 main attempts remaining. Reusing any of it requires an applicable registration and serial resource gate; a new training campaign requires a new budget. Do not turn a remaining ceiling into a target sample count.
...
## Current next step after public selection

The [six-case scene screen](SELECTION_BOUNDARY_ADMISSION02_RESULTS_zh.md) is complete: continuous and Linear29 each pass the shifted beam, contact the lower beam and time out outside the farther goal. All three paired outcome categories match; this does not establish equivalence. Admission 02 adds four attempts / 1,148 steps, for six total / 1,867 steps. The earlier two-attempt resource-deferral packet remains immutable.

The sibling's newer controller now supports one distance-dependent blended exit. It uses a fixed composed clock, historical stopping estimates and one public choice; its eight successful controls/public trials contain three distinct action histories. It is a separate package with a narrow goal050 result, not a matched improvement over the old selector or generic braking.

The [four-case comparison is complete](DISTANCE_CODEC_ADMISSION02_RESULTS_zh.md): continuous 2/2, Linear29 1/2, four new attempts / 1,660 steps / 6,640 physics samples. Original-goal tasks both pass. At the farther goal, Linear29 requests the loop but misses the fixed clearance gate at tick 126 by 1.069 mm; the gate opens at 134 and is never rechecked. It stops outside the goal, without contact or a fall. All scores/references and control/initial-state audits match exactly. The original zero-launch deferral remains unchanged; the four-attempt budget is exhausted.

The [pending-exit experiment](PENDING_EXIT_RESULTS_zh.md) is now implemented, registered and partially executed. Its array proof gives a common last acceptance tick of 162, with five dense control frames committed and later forecasts revisable. Disabled replay preserves all 1,660 incumbent states. Three new retention controls pass and reproduce their old complete trajectories exactly: 1,160 control steps / 4,640 physics samples. The decisive farther-goal Linear29 case never launched. The queue received SIGTERM during its memory wait; sender unknown, with no normal timeout receipt. This is one queue interruption, not a fourth native failure or an executed repair.

[Admission 02](PENDING_EXIT_ADMISSION02_RESULTS_zh.md) now preserves those 428 bindings and reused controls but closes normally after 300 seconds with zero launches: none of 15 samples qualifies; GPU memory peaks at 11,218 MiB against 12,000 required. Keep this timeout distinct from the prior SIGTERM. First check resource availability, then register only the remaining case in a new admission with unchanged one-attempt / 500-tick limits. The original four-attempt ceiling still has one unused attempt. Actual delayed Linear29 execution remains the decisive priority.

The sibling at `eb8f1f8c` separately qualifies executed-endpoint calibration: 12/12 versus 11/12, one gain and no regressions, with 16 new and eight reused runs. Its narrowest successful 3D goal margin is 1.658 cm. This model was not substituted into our frozen comparison. Post-result calibrated shadows still block the loop on actual Linear29 histories. Endpoint calibration must declare controller, codec and state provenance; continuous fit does not establish decoded-loop support. See the [sync and exploratory audit](DISTANCE_CODEC_ADMISSION02_RESULTS_zh.md), and the [reporting correction](DISTANCE_METRIC_CORRECTION_20260919.md): the frozen success radius is 3D, while the predictor uses XY.

Lower-ceiling coverage, blocked-state execution, broader incoming-state coverage and sensing remain open. The neighboring reset screen has now completed: 5/6 beam and 6/6 clear changed-state successes, plus two exact zero controls. Continuous motion from a backward start also misses the fixed gate and clears later; two successes have millimetre-scale goal margins. This supports diagnosing a shared temporal interface weakness, not treating the failure as inherent to quantization. Its [committed pending comparison](PENDING_CONTROLLER_SYNC_20260920_zh.md) now passes both the delayed public run (acceptance 138) and an early supplied-loop control. These calibrated continuous results are separate from our original-predictor Linear29 comparison and tick-162 window. After our remaining case, a compatible whole-episode inventory and small causal learning protocol are the next candidate artifacts; no fit is registered here.
