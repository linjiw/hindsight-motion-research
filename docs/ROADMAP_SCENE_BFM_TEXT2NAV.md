...
| P1 — complete task baseline | Continuous × Linear29 across full-reference reset, matched-state suffix, and causal-composer reset conditions; native parity separately | Full-reference row: 4/4 per method; same-phase continuation subset: 8/8 per method, eight exactly matched handoff pairs. Familiar known-map selector subset: 2/2 per method. New scenes, perturbations and cross-family composition remain untested |
...
Both simple routes complete the selected known references, all eight same-phase handoffs, and both familiar public-selector tasks. No complete-task success bottleneck has been isolated for Linear29 in these panels. Next freeze the controller and codec for new beam placements: provisionally ±5 cm along travel from the present beam, plus one shared clear control, six primary cells after a new registration. Keep all fixed outcomes. Vary incoming state, speed, dimensions and disturbances separately; cross-family stitching and sensor execution remain open. The first pilot used the existing goal050 profile; do not silently relabel it goal025. Frozen Linear29 ranges clipped four wrist channels, so later failures need calibration-versus-temporal-loss controls. See the [complete-task report](COMPLETE_TASK_RESULTS_zh.md).
