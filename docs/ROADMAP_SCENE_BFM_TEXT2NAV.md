...
Our [offline codec preflight](DISTANCE_CODEC_PREFLIGHT_zh.md) exactly replays all 1,528 public-history references. Linear29 keeps the five-frame commitment but moves the first goal-dependent forecast difference from control tick 127 to 126; the blend also has a provided-velocity versus position-FD difference. Next register original/farther beam × continuous/Linear29 on this frozen controller, explicitly retaining these raw-codec semantics. Audit each method's actual clearance decision and complete task; same-history shadow outputs do not qualify free-running execution. Separate later velocity/boundary controls if a codec-only failure appears. Keep lower-ceiling coverage, blocked-state execution, reset diversity, the sibling's wider distance envelope and sensing as separate pending work. No new tokenizer or learner fit is justified yet.
