...
Our [offline codec preflight](DISTANCE_CODEC_PREFLIGHT_zh.md) exactly replays 1,528 public-history references. The [four-case comparison is now registered and prepared](DISTANCE_CODEC_RESULTS_zh.md), with 792 beam states replayed and decoded arrays re-derived exactly. The 300-second resource gate expired without a launch; all four cases remain unrun. Next use a separate admission preserving the original deferral, frozen inputs and total four-attempt ceiling. Raw Linear29 keeps the five-frame commitment but shifts forecast timing and velocity convention; physical choice and task retention remain unmeasured.

The [new sibling envelope](DISTANCE_ENVELOPE_SYNC_20260919_zh.md) resolves the missing matched +0.60 m short-only baseline and qualifies nominal-loop execution. Public selection passes 9/10 versus short-only 5/10, with five gains and one +0.45 m regression. Its proposed executed-loop endpoint calibration is an offline hypothesis. Keep controller endpoint prediction separate from codec retention, and do not change the registered predictor to fit that new result. Lower-ceiling coverage, blocked-state execution, reset diversity and sensing remain open. No new tokenizer or learner fit is justified yet.
