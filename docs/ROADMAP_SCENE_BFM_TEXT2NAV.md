...
The [next proposed experiment](CLEARANCE_ADMISSION_DIAGNOSTIC_zh.md) isolates request timing: latch the original distance request and permit clearance admission only within an audited common commitment window. Keep the codec, motor, endpoint predictor, 2 cm clearance and complete-task thresholds unchanged. Prove the prefix contract before registering four new cases with the present four incumbent cases reused. Later gate opening is an offline observation, not an executed repair. A future loop blend cannot cause a failure before its reference is issued; only follow the causal past when choosing component controls.
