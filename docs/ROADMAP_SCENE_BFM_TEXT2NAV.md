...
Next admit only that never-launched case in a new packet, binding the interruption and all completed controls. At most one new attempt / 500 ticks / 2,000 physics samples remains, under the same resource gate and total four-attempt ceiling. Reuse the qualified farther continuous run instead of repeating it. Keep codec, predictor, motor and thresholds unchanged. A shadow accepts at 134 and changes q/qdot forecasts first at +0.70 s; actual delayed execution is still required before interpreting the timing hypothesis.
...
Lower-ceiling coverage, blocked-state execution, broader incoming-state coverage and sensing remain open. The neighboring reset screen has now completed: 5/6 beam and 6/6 clear changed-state successes, plus two exact zero controls. Continuous motion from a backward start also misses the fixed gate and clears later; two successes have millimetre-scale goal margins. This supports diagnosing a shared temporal interface weakness, not treating the failure as inherent to quantization. Its proposed pending-exit wrapper should be reused where compatible, with any revised forecast/commitment semantics declared explicitly. No new tokenizer or learner fit is justified yet.
